...
def main() -> None:
    ...
    for ff in range(len(pos_files)):
        ...
        noise_file_path = OUTPUT_DIR / f"noisy_data_{ff+1:03}.csv"
        noise_ = pd.read_csv(noise_file_path)
        noise_data.append(np.array([list(noise_[f"s{yy+1}"])[-1] for yy in range(len(noise_.columns)-1)]))
    ...
    if PLOT_RES:
        ...
        axs[0].boxplot(sim_data,sym="")
        axs[0].set_title("Perfect sensors")
        axs[0].set_xlabel("Sensor #")
        axs[0].set_ylabel(r"Temperature [$\degree$C]")
        axs[1].boxplot(noise_data,sym="")
        axs[1].set_title("Noisy sensors")
        fig.show()
        
    avm(sim_data[:,-1],noise_data[:,-1][::100])
    mavm(sim_data[:,-1],noise_data[:,-1][::100])
