...
def main() -> None:
    ...
    cuDensity = [8.91e+03,8.92e+03,8.93e+03,8.96e+03,8.90e+03]
    cuThermCond = [401,398,393,403,395] #W/m/k # 398 W/(m·K) with uncertainty of 5 W/(m·K)
    cuSpecHeat = [0.386,0.376,0.380,0.390,0.385]
    ...
    if save_vals:
        var_path = Path("examples/images/moose_vars.csv")
    ...
    if read_results:
        for nn in range(len(moose_vars)):
            ...
            pos_noise = np.random.normal(loc=0.0,scale=0.1,size=sens_pos.shape) # Gaussian dist +/- 0.1 (sim dimensions)
            sens_pos = sens_pos + pos_noise
        ...
        #     offset. The max temp in the simulation is ~200degC so this range [lo,hi]
            ...
            if save_vals:
                ...
                print(sens_pos_df)
                pos_path = Path(f'examples/images/sens_pos_{nn+1}.csv')
                ...
                data_path = Path(f'examples/images/sim_data_{nn+1}.csv')
