...
__global__ void raytrace_gpu(
    ...
    if (idx < num_rays){                
        ...
        int y = (idx / samples_per_pixel) / buffer_width_gpu; 
        int x = (idx / samples_per_pixel) % buffer_width_gpu; 

        // get centre of each pixel in world coordinates
        double vec[3];
        vec[0] =  (x + random_samples[idx]) * pixel_size_gpu - (buffer_width_gpu  * pixel_size_gpu) / 2.0; 
        vec[1] = -(y + random_samples[idx]) * pixel_size_gpu + (buffer_height_gpu * pixel_size_gpu) / 2.0;  
        vec[2] = -focal_length_gpu;

        if (idx / samples_per_pixel == 0){
            printf("elem_x: %f %f %f\n", vec[0] ,vec[1] ,vec[2]);
        }
        ...
        for (int elem = 0; elem < num_elems_in_scene; elem++){
            ...
            if ((intersect(cam_pos_world, direction, elem_x, elem_y, elem_z, cam_to_intersect)) && (cam_to_intersect < nearest_intersect)){
                ...
                atomicAdd(&depth_buffer[idx / samples_per_pixel], 1.0 / samples_per_pixel);

            } 
        }

        // depth_buffer[idx / samples_per_pixel] += nearest_intersect / samples_per_pixel;
...
void raytrace_gpu_setup(
    ...
    MEASURE_TIME("Cuda Memory Allcation:", {
        ...
        CUDA_CALL(cudaMalloc((void**)&curandom::pixel_samples,  sizeof(double) * num_pixels * samples_per_pixel));
    ...
    int n_blocks = (num_rays + threads_per_block - 1) / threads_per_block;


    MEASURE_TIME("Cuda Memory copy from CPU to GPU:", {
        curandom::setup_curand_generator();
        curandom::generate_pixel_samples(num_rays);
    });

    MEASURE_TIME("Cuda kernel run time:", ({ // added an extra bracket because of the comma in the kernel call messing with the macro.
        raytrace_gpu<<<n_blocks,threads_per_block>>>(num_rays, num_elems_in_scene, curandom::pixel_samples, cam_pos_world_gpu, cam_rot_gpu, coords_world_gpu, depth_buffer_gpu);
