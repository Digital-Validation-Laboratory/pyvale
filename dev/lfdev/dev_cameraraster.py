...
def main() -> None:
    ...
    cam_pos_world = np.array([roi_pos_world[xx] - image_dist*np.sin(phi_y),
                              roi_pos_world[yy],
                              image_dist*np.cos(phi_y)])
    dist_cam_to_roi = np.linalg.norm(cam_pos_world - roi_pos_world)
    cam_rot = Rotation.from_euler("zyx", [0, phi_y, 0], degrees=True)
    ...
    px_coord_flat_x = px_coord_grid_x.flatten()
    px_coord_flat_y = px_coord_grid_y.flatten()
    px_coords = np.vstack((px_coord_flat_x,px_coord_flat_y))
    ...
    (aa,bb,cc) = (0,1,2)
    # elem_areas = ((elem_raster_coords[cc,xx,:] - elem_raster_coords[aa,xx,:])
    #              *(elem_raster_coords[bb,yy,:] - elem_raster_coords[aa,yy,:])
    #              -(elem_raster_coords[cc,yy,:] - elem_raster_coords[aa,yy,:])
    #              *(elem_raster_coords[bb,xx,:] - elem_raster_coords[aa,xx,:]))
    ...
    for ee in range(num_elems_in_scene):
        # Each bound box should have 625 pixels for each of our 5mm edged triangles
